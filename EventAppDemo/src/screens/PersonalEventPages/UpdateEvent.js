...
export default function UpdateEvent() {
  ...
  const [dateVisible, setDateVisible] = useState(item.dateToShow);
  const [dateStore, setDateStore] = useState(item.date);
    ...
    try {
      if (mode === "date") {
        if (checkForValidDate(props)) {
          ...
          // console.log(getDate(props, "Month"));
          // console.log(getDateStr(props));
          setDateVisible(getDate(props, "Month"));
  ...
  const updateEvent = () => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        ...
        (tx, results) => {
          Alert.alert("Status", "Event Updated Successfully", [
            {
              text: "OK",
              onPress: () => {
                navigation.goBack();
  ...
  return (
    <SafeAreaView style={CommonStyles.container}>
      ...
      <View height={marginBetween} />
      <CustomButton
        titleVal="Update Event"
        onPressClbck={updateEventCallBack}
      />
    </SafeAreaView>
  );
}
